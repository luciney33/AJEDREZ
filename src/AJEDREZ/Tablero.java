...
public class Tablero {
    ...
    public void mover(Movimiento mov) {
        ...
        // ⚠️ No permito capturar al rey porque en ajedrez no se puede,
        // aunque no implementé jaque ni jaque mate.
        if (destino != null && destino instanceof Rey) {
            System.out.println("❌ No puedes capturar al rey.");
            return;
        }
            ...
            if ((pieza.getColor() && filaFinal == 0) || (!pieza.getColor() && filaFinal == 7)) {
                ...
                switch (eleccion) {
                    ...
                    case "C":
                        ...
                        break;
                    default:
                        System.out.println("❌ Opción no válida, el peón se convertirá en una Dama por defecto.");
                        pieza = new Dama(pieza.getColor());
                }
            }
        }
        ...
        // Verificar movimiento diagonal
            ...
            while (fila != filaFinal && columna != columnaFinal) {
                if (Tablero[fila][columna] != null) { // Si hay una pieza en el camino
                    System.out.println("❌ Hay una pieza bloqueando en: (" + fila + "," + columna + ")");
                    return true; // Hay una obstrucción
                }
                fila += pasoFila;
                columna += pasoColumna;
